...
const formatDisplayedAllianceScoreLabel = (rawScore: number) => {
  const safe = Math.max(0, Number(rawScore || 0));
  if (safe <= 0) return "0";
  const scaled = Math.max(0.01, Math.round((safe / SCORE_DISPLAY_DIVISOR) * 100) / 100);
          ...
          return (
            <article key={item.id} className="alliance-search-card">
              ...
              <div className="alliance-search-footer">
                <strong>{l("Score alliance", "Alliance score")}: {formatDisplayedAllianceScoreLabel(Number(item.publicStats?.pointsTotauxAlliance || 0))}</strong>
  ...
  const renderOverview = () => (
    <>
      <section className="alliance-dashboard-grid">
        ...
        <article className="alliance-stat-card"><small>{l("Classement global", "Global ranking")}</small><strong>{rankLabel(Number(allianceData?.rank || 0))}</strong><span>{formatDisplayedAllianceScoreLabel(Number(publicStats.pointsTotauxAlliance || 0))}</span></article>
